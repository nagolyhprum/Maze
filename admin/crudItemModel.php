			...
			if($c) {
				$stmt = mysqli_prepare($c, "
					...
					INNER JOIN
						Statistic as s
					ON
						i.StatisticID=s.StatisticID					
				");		
				$statistic = array();
				mysqli_stmt_bind_result($stmt, $id, $name, $area, $weight, $attacktype, $itemtype, $portrait, $statistic["id"], $statistic["health"], $statistic["energy"], $statistic["strength"], $statistic["defense"], $statistic["intelligence"], $statistic["resistance"]);
				mysqli_stmt_execute($stmt);
